...
def _fit(model, rows, labels, config, torch, device=None, validation_rows=None):
    ...
    inputs = _tensor(rows, torch, device)
    expected = torch.tensor([label_to_id[row["label"]] for row in rows], dtype=torch.long, device=device)
    counts = torch.bincount(expected, minlength=len(labels)).float()
    class_weights = expected.numel() / (len(labels) * counts.clamp_min(1.0))
    ...
    val_inputs = val_expected = val_labels = None
    if validate:
        val_inputs = _tensor(validation_rows, torch, device)
        val_expected = torch.tensor([label_to_id[row["label"]] for row in validation_rows], dtype=torch.long, device=device)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        loss_t = torch.nn.functional.cross_entropy(model(*inputs), expected, weight=class_weights,
        ...
        if validate:
            model.eval()
            with torch.no_grad():
                val_logits_t = model(*val_inputs)
                val_loss = float(torch.nn.functional.cross_entropy(val_logits_t, val_expected).item())
...
def _logits(model, rows, torch, device=None):
    with torch.no_grad():
        return model(*_tensor(rows, torch, device)).cpu().numpy()
...
def evaluate_sealed_once(model, rows, labels, torch, *, temperature=1.0, reject_id=None, top_threshold=0.0, margin=0.0, on_evaluate=None, device=None):
    if on_evaluate is not None:
        on_evaluate("sealed")
    expected = np.array([labels[row["label"]] for row in rows], dtype=np.int64)
    return evaluate(_logits(model, rows, torch, device), expected, temperature, reject_id=reject_id, top_threshold=top_threshold, margin=margin)
...
def main(argv=None):
    ...
    def evaluate_fold(model, validation_rows):
        ...
        return evaluate(_logits(model, validation_rows, torch, device), expected)
    ...
    calibration_labels = np.array([label_to_id[row["label"]] for row in calibration_rows], dtype=np.int64); calibration_logits = _logits(selected, calibration_rows, torch, device)
    ...
    metrics = evaluate_sealed_once(selected, test_rows, label_to_id, torch, temperature=temperature, reject_id=label_to_id["reject"], top_threshold=top_threshold, margin=margin, device=device)
