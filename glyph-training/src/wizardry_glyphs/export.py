...
def export_bundle(model, inputs, output: Path, labels, *, metadata=None):
    ...
    model.eval()
    torch.onnx.export(
        model, inputs, onnx_path, opset_version=17,
        input_names=["vectors", "mask", "raster"], output_names=["logits"],
        dynamic_axes={"vectors": {0: "batch"}, "mask": {0: "batch"}, "raster": {0: "batch"}, "logits": {0: "batch"}},
    )
    with torch.no_grad():
        golden = model(*inputs).detach().cpu().tolist()
    files = {"model.onnx": _sha256(onnx_path)}
    metadata = metadata or {}
    manifest = {
        "schema_id": metadata.get("schema_id", "glyph-bundle-v1"),
        "model_id": metadata.get("model_id", "fused-glyph-v1-fixture"),
        "catalog_id": metadata.get("catalog_id", "catalog-fixture"),
        ...
        "input_schema": {
            ...
            "mask": {"shape": [None, 64, 32], "dtype": "float32"},
            "raster": {"shape": [None, 1, 64, 64], "dtype": "float32"},
        },
    ...
    (output / "sha256sums.json").write_text(json.dumps(files, sort_keys=True, indent=2))
    return manifest
