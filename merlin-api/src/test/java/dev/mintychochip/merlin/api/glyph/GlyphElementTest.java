...
final class GlyphElementTest {
    @Test void paletteMatchesLockedHex() {
        assertArrayEquals(new float[] {1f, 77 / 255f, 0f}, GlyphElement.FLAME.rgb(), 1e-6f);
    ...
    @Test void blendIsCoverageMix() {
        ...
        GlyphElement.blend(pixel, 0, GlyphElement.FLAME, 1f);
        assertArrayEquals(GlyphElement.FLAME.rgb(), pixel, 1e-6f);
        GlyphElement.blend(pixel, 0, GlyphElement.FROST, 0.5f);
        assertEquals((GlyphElement.FLAME.r() + GlyphElement.FROST.r()) / 2f, pixel[0], 1e-5f);
        assertEquals((GlyphElement.FLAME.g() + GlyphElement.FROST.g()) / 2f, pixel[1], 1e-5f);
        assertEquals((GlyphElement.FLAME.b() + GlyphElement.FROST.b()) / 2f, pixel[2], 1e-5f);
    }
