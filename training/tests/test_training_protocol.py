...
def test_fit_with_validation_returns_history_and_best_checkpoint():
    import torch, numpy as np
    from wizardry_glyphs.model import FusedClassifier
    from wizardry_glyphs.train import _fit
    model = FusedClassifier(classes=2, embedding_dim=2)
    ...
    assert "model" in result and "history" in result
    assert len(result["history"]) == 3
    assert all("val_macro_f1" in h for h in result["history"])
