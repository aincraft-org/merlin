...
public final class LavaWalkerHandler implements OvercapEffectHandler, PlayerMoveTrigger {
    private static final NamespacedKey KEY = CustomEnchantmentSupport.customKey("lava_walker");
    ...
    public void onPlayerMove(Player player, Location from, Location to, int level) {
        ...
        for (int dx = -RADIUS; dx <= RADIUS; dx++) {
            for (int dz = -RADIUS; dz <= RADIUS; dz++) {
                ...
                if (below.getType() == Material.AIR || below.getType() == Material.LAVA) continue;
